...
class Unidad extends CI_Controller {
	...
	public function contenido_pagina(){
		...
		if(!count(@$data['buses']['pagina_web']))redirect(base_url('buses/unidad/'));
	...
	public function registro(){
		...
		$id_pagina = $this->uri->segment(4);
		if(!$id_pagina)redirect(base_url('buses/unidad/'));
	...
	public function editar(){
		...
		if(!$id_bus)redirect(base_url('buses/unidad/'));
		$data['bus'] = $this->modelo_buses->buscar_bus($id_bus);
		//var_dump($data); exit;
		if(!count($data['bus']['id_bus']))redirect(base_url('buses/unidad/'));
	...
	public function copiar(){
		...
		if(!$data['bus_copiado'])redirect(base_url('buses/unidad/'));
	...
	public function buses_relacionados(){
		...
		if(!$id_codigo_bus)redirect(base_url('buses/unidad/'));
		//$pagina_web = $this->modelo_buses->buscar_pagina($id_pagina);
		//if(!count($pagina_web))redirect(base_url('buses/unidad/'));
		$data['buses_relacionados'] = $this->modelo_buses->retorna_buses_codigo($id_codigo_bus);
		if(!count($data['buses_relacionados']))redirect(base_url('buses/unidad/'));
