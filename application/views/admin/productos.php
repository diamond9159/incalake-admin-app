<?php
  ...
  if(isset($producto_relacionado)){
   ...
   //var_dump($idiomas);
  $idiomasJSON = array();
  foreach($idiomas as $value)if($value['codigo']!='ES')$idiomasJSON[strtolower($value['codigo'])] = strtolower($value['pais']);
//var_dump($servicio);
  ?>
<!DOCTYPE html>
<html lang="es">
...
<script src="<?=base_url();?>assets/resources/bootstrap-select/js/bootstrap-select.min.js" type="text/javascript"></script>
...
<script>
    ...
    echo 'idiomas='.json_encode($idiomasJSON).';';//imprimir idiomas en json para precios.js
      ...
      function addInicios($inicio=null,$duracion=null,$tipo=null,$zona=null){
         ...
         foreach($options as $key => $value) $optionsHTML .= '<option '.($tipo==$key&&$tipo!=null?'selected':'').' value="'.$key.'">'.$value.'</option>';
      ...
      //$idtab = 0;
      function addTab($nombre=null,$icono=null,$html=null,$textareaIDkey=false){// si tiene valor trabaja con php
             $fontawesome = array('address-book'      => 'f2b9',
                                  'car'               => 'f1b9',
                                  'bars'              => 'f0c9',
                                  'clock-o'           => 'f017',
                                  'commenting'        => 'f27a',
   ...
   function manejarDOM(){
     ...
     $('.zona_check').each(cambiarBandera);
      ...
      $('#addSalidasButton').click(function(){
        ...
        else $('#added_salidas').append(varhtml);$(".div-sin-horario").remove();
        $('.inputTime').timepicker({
                  minuteStep: 10,
                  appendWidgetTo: 'body',
                  showSeconds: false,
...
/**********************************************************/   
   ...
   $('#btn_listo').click(function(){
    var inputs = $('#contenedorGenerador').find('input, select');
        inputs.focusin(function(){$(this).css('border-color','#CCC');});
    ...
    var exito = true;
      ...
      if(exito){
        ...
        var desde_hora = desde_tiempo=='AM'?parseInt(desde_horamin[0]):parseInt(desde_horamin[0])+12;
        var hasta_hora = hasta_tiempo=='AM'?parseInt(hasta_horamin[0]):parseInt(hasta_horamin[0])+12;
        ...
        if(desde_hora<=hasta_hora){
          for(i=desde_hora;i<=hasta_hora;i+=parseInt(inputs[2].value)){
            ...
            inputs2[0].value=(i<13?i:i-12)+':'+desde_horamin[1]+(i<13?' AM':' PM');
...
/**********************************************************/
     eliminarinicios=function (elemen){
      ...
      if($("#added_salidas").html()==""){
        $("#added_salidas").html('<?=$div_notiene_horarios;?>');clicksSalidas=false;
        console.log('vacio');
      }
     }
...
<content>
  <div class="container-fluid">
     <div class="titulo-producto">ACTIVIDAD<span class="fa fa-chevron-right" style="vertical-align: middle;color: #5cb85c;"> </span> <?=$tituloProducto;?></div>
     <div class="alert alert-info" style="padding:7px">
      <b>Idioma: </b><span class="flag flag-<?=@$servicio['codigo']=='EN'?'us':strtolower($servicio['codigo']);?>"></span> <?=@$servicio['pais'];?> | <strong>URL: </strong> <?=@$servicio['url_servicio'];?>
      
      <?php
                if($formButton=='Actualizar'){
              ?>
              
                <button style="margin-top:-7px" type="button"  class="btn btn-success pull-right" onclick="$('#btnGuardar').trigger('click');">Actualizar y guardar</button>
               <?php };
     ?>
     </div>
      <?php
     ...
     <input type="hidden" name="codigo_prod_2" value="<?=@$producto['id_codigo_producto'];?>" /> <!-- para agrupar por idiomas -->
                                                    ...
                                                    <div class="text-left">
                                                      <div class="headline">
                                                       ...
                                                       <div class="form-group row">
                                                          <label class="col-md-2" for="titulo_prod">Titulo actividad <span class="btn btn-info btn-xs fa fa-question" data-placement="top" data-toggle="popover"  data-content="Escribir el título de la actividad ubicado en el encabezado."></label>
                                                          <div class="col-md-10">
                                                             <input required class="form-control" data-error="Necesita ingregar el titulo de la actividad." type="text" id="titulo_prod" name="titulo_prod" value="<?=@$producto['titulo_producto'];?>" autocomplete="off"/>
                                                          </div>
                                                       </div>
                                                       <div class="form-group row">
                                                          <label class="col-md-2" for="subtitulo_prod">Subtitulo actividad <span class="btn btn-info btn-xs fa fa-question" data-placement="top" data-toggle="popover"  data-content="Escribir el subtítulo de la actividad ubicado bajo el encabezado del título de actividad."></label>
                                                          <div class="col-md-10">
                                                             <input  class="form-control" id="subtitulo_prod" name="subtitulo_prod" type="text" value="<?=@$producto['subtitulo_producto'];?>" />
                                                          </div>
                                                       </div>
                                                       <div class="form-group row">
                                                          <label class="col-md-2" for="codigo_prod">Codigo de actividad <span class="btn btn-info btn-xs fa fa-question" data-placement="top" data-toggle="popover"  data-content="Escribir el código de la actividad de uso personal para el administrador."></label>
                                                          <div class="col-md-10">
                                                             <input required  placeholder="Ejemplo: UROS-MD-2017" data-error="Ingrese el Codigo de la actividad." <?=(isset($producto) or isset($producto_relacionado))?'readonly':'';?> class="form-control" id="codigo_prod" name="codigo_prod" type="text" value="<?=@$producto['codigo_producto'];?>" />
                                                          </div>

                                                       </div>
                                                       <div class="form-group row">
                                                          <label class="col-md-2" for="ciudad_cercana">Ubicación de la Actividad Turistica<span class="btn btn-info btn-xs fa fa-question" data-placement="top" data-toggle="popover"  data-content="Indique el nombre o la ciudad donde esta ubicado la actividad Turistica"></label>
                                                          <div class="col-md-10">
                                                             <input required class="form-control" id="ciudad_cercana" name="ciudad_cercana" type="text" value="<?=@$producto['ciudad_cercana'];?>" />
                                                          </div>
                                                       </div>
                                                       <div class="form-group row">
                                                          <label class="col-md-2" for="aeropuerto_cercano">Aeropuerto cercano <small>(OPCIONAL)</small> <span class="btn btn-info btn-xs fa fa-question" data-placement="top" data-toggle="popover"  data-content="Escribir areopuerto cercano al producto."></label>
                                                          <div class="col-md-10">
                                                             <input class="form-control" id="aeropuerto_cercano" name="aeropuerto_cercano" type="text" value="<?=@$producto['aeropuerto_cercano'];?>" />
                                                          </div>
                                                       </div>
                                                       <div class="form-group row">
                                                          <label class="col-md-2" for="aeropuerto_cercano">Actividad Activo <span class="btn btn-info btn-xs fa fa-question" data-placement="top" data-toggle="popover"  data-content="Estado de producto  visible/no vivisble, en la web."></label>
                                                          <div class="col-md-10">
                                                             <input type="checkbox" name="estado_producto" value="1" <?=!isset($producto['estado_producto'])?'checked':((int)$producto['estado_producto']?'checked':'');?> /><small> Si esta activo este producto se muetra en la web de lo contrario no.</small>
                                                       ...
                                                       <div class="col-md-12 form-group row">
                                                                ...
                                                                <div class="col-md-12 text-center">
                                                                   <button type="button" class="btn btn-success" id="addSalidasButton" title="(Usado mayormente en tours compartidos o de salida simple)"><span class="fa fa-plus"></span> Agregar Hora de salida</button>
                                                                  <button type="button" class="btn btn-warning" id="generarSalidas" title="(Usado mayormente en tours privados)"> Generar rango de salidas</button>
<div><small>Agregar Hora de salida (Usado mayormente en tours compartidos o de salida simple) | Generar rango de salidas (Usado mayormente en tours privados)</small></div>
                                                                  ...
                                                                  <div id="contenedorGenerador">
                                                                    <div class="col-md-1" style="width:50px">
                                                                     ...
                                                                     <div class="col-md-1">
                                                                     <input readonly type="text" value="7:00 AM" class="form-control inputTime" />
                                                                     </div>
                                                                     <div class="col-md-1" style="width:50px">
                                                                     <label>Hasta: </label>
                                                                    </div>
                                                                     <div class="col-md-1">
                                                                     <input readonly type="text"  value="11:00 AM" class="form-control inputTime" />
                                                                     </div>
                                                                    ...
                                                                    <div class="col-md-2">
                                                                     ...
                                                                     <div class="col-md-1">
                                                                      ...
                                                                      <div class="col-md-2">
                                                                        ...
                                                                        <button type="button" id="btn_listo" class="btn btn-info"><i class="fa fa-check" aria-hidden="true"></i></button> 
                                                                         <button onclick="$('#generarSalidas').trigger('click')" type="button" id="btn_listo" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></button>
                                                                     </div><br><br>
                                 ...
                                 <div class="tab-content">
                                             ...
                                             <div class="typography">
                                                 ...
                                                 <div class="col-md-12">
                                                     <div class="headline">
                                                        ...
                                                        <textarea class="form-control" name="descripcion_prod" placeholder="descripcion del tour" id="txt_descripcion"><?=@$producto['descripcion_tab'];?></textarea>
                                                  </div>
                                         ...
                                         <div class="typography">
                                             <div class="col-md-12">
                                                <div class="headline">
                                                  ...
                                                  <textarea class="form-control" name="itinerario_prod" placeholder="Itinerario" id="txt_itinerario"><?=@$producto['itinerario_ta'];?></textarea>
                                             </div>
                                         </div>
                                     </div>
                                         ...
                                         <div class="typography">
                                             <div class="col-md-12">
                                                <div class="headline">
                                                  ...
                                                  <textarea class="form-control" name="incluye_prod" placeholder="Type the things that includes" id="txt_incluye"><?=@$producto['incluye_tab'];?></textarea>
                                             </div>
                                         </div>
                                     </div>
                                     <div class="tab-pane fade" id="div_mapa">
                                         <div class="typography">
                                             ...
                                             <div class="row">
                                               <div class="col-md-8 col-sm-8 col-xs-12">
                                                 <div class="text-justify">
                                                  <p>Seleccione la ruta del servicio, ponga los destinos a conocer</p>
                                                  <p><strong>Ejemplo: </strong> Un vistante turistico una vez llegado a Puno tiene programado visitar a la Isla de Los Uros y en seguida visitar la Isla amantaní y finalmente a la Isla de Taquile.</p>
                                                  <p>Para el ejemplo anterior se recomienda ingresar primero La Isla de los Uros, después la Isla de Amantaní y al último a la Isla de taquile; El orden del regisgtro de lugares turísticos se debe realizar de acuerdo al orden de visitas.</p>
                                                 </div>
                                            ...
                                            <textarea class="form-control" readonly name="map_prod" id="map_prod" placeholder="mapa del tour"><?=@$producto['mapa_tab'];?></textarea>
                                            </div>
                                         </div>
                                     </div>
                                     <div class="tab-pane fade" id="div_informacion_importante">
                                         <div class="typography">
                                            <div class="col-md-12">
                                               ...
                                               <textarea class="form-control" name="info_prod" placeholder="informacion del tour" id="txt_informacion_adicional"><?=@$producto['informacion_tab'];?></textarea>
                                            </div>
                                         </div>
                                     </div>
                                     ...
                                     <div class="tab-pane fade" id="div_recomendacion">
                                         <div class="typography">
                                            <div class="col-md-12">
                                               ...
                                               <textarea class="form-control" name="recomendaciones_prod" placeholder="recomendaciones" id="txt_recomendaiciones"><?=@$producto['recomendacion_tab'];?></textarea>
                                            </div>
                                         </div>
                                     </div>
                                     ...
                                     <div class="tab-pane fade" id="div_add_tabs" > 
                                            ...
                                            <button id="addTabButton" class="btn btn-primary"><span class="fa fa-plus"></span> Agregar Tab</button>
                                                  ...
                                                  <div class="col-md-6">
                                                     <div class="panel panel-primary panel-slider">
                                                           <div class="panel-heading">
                                                                ...
                                                                <b>GALERIA DEL SLIDER</b><span style="float:right;margin-top:-7px" class="btn btn-success btn-sm addGaleriaSlider"><span class="fa fa-plus-circle"></span></span>
                                                                ...
                                                                <?php
                                                                   if(isset($producto)){
                                                                       ...
                                                                       $html_galeria = null;//esta variable se usa rtambien abajo para detectar si hay imagenes agregadas
                                                                         ...
                                                                         echo $html_galeria;
                                                                    } else echo 'Este producto no tiene imagenes en su galeria (+) para agregar Fotos.';
                                                                   } else echo 'Click en el Boton (+) para agregar Fotos.';
  
                                                                 ?>
                                                             </div>
                                                             <div><button class="center-block btn btn-primary addGaleriaSlider"><span class="fa fa-plus-circle"></span> Agregar Campo de Slider</button><br></div>
<div class="alert bg-primary" style="margin: 0;">
                                                              <div>
                                                                <p><b> NOTA!</b></p>
                                                              
                                                                La <b>
                                                               primera imagen</b> de la lista correspondera a la <b>imagen descata</b> de la actividad</div>
                                                              </div>
                                                         </div>
                                             </div>
                  <div class="col-md-6">
                              <div class="panel panel-primary">
                                                           <div class="panel-heading">
                                                                ...
                                                                <span class="fa fa-file-image-o"></span>
                                                                <b>ARCHIVOS ADJUNTOS</b><span style="float:right;margin-top:-7px" class="btn btn-success btn-sm addGaleriaAdjunto"><span class="fa fa-plus-circle"></span></span>
                                                                   ...
                                                                   if(!empty($producto['adjuntos_producto'])){
                                                                    //var_dump($producto['adjuntos_producto']);
                                                                      ...
                                                                      // var_dump($producto['adjuntos_producto']);
                                                                         ...
                                                                         echo $html_galeria;
                                                                    } else echo 'Este producto no tiene archivos adjuntos en su galeria pulse (+) para agregarlos.';
                                                                   } else echo 'Click en el Boton (+) para agregar Adjuntos.';
  
                                                              
  
                                                                 ?>
                                                             </div>
                                                             <div><button class="center-block btn btn-primary addGaleriaAdjunto"><span class="fa fa-plus-circle"></span> Agregar Campo de Adjunto</button><br></div>
                                                         </div>
                   <!--div>
                                      ...
                                      <div class="headline">
                                            <div>Paso 4: Precios generales
                                             ...
                                             <table style="display: inline;" class="pull-right">
                                                  <tr>
                                                      <td style="font-size: 14px;font-weight: bold;background: #ffffbd;"> Tasas e impuestos a aplicar &nbsp; &nbsp; </td>
                                                      <td> <input type="text" name="tasas_impuestos" value="<?=@$producto['tasas_impuestos']?$producto['tasas_impuestos']:5 ?>" style="width:40px;text-align: center;font-weight: bold;" /> %</td>
                                                      <td style="font-size: 14px;font-weight: bold;background: #d2ffe9;"> Porcentaje de primera cuota &nbsp; &nbsp; </td>
                                                      <td> <input type="number" required name="porcentaje_adelanto" value="<?=@$producto['porcentaje_adelanto']?$producto['porcentaje_adelanto']:20 ?>" style="width:40px;text-align: center;font-weight: bold;" /> %</td>
                                                  </tr>
                                              </table>
                                         </div>
                                                 ...
                                                 <div class="col-md-12">
                                                     ...
                                                     <div class="form-group row">
                                                       ...
                                                       <div class="col-md-12" >
                                                           ...
                                                           <div id="preciosManager" data-json='<?=@$producto['precios_full'];?>' class="col-md-12">
                                                             <ul class="nav nav-tabs"><li><a class="addTabBtn fa fa-plus" title="Agregar nuevo Segmento"></a></li></ul>
          ...
          <fieldset>
              <div class="panel panel-primary navform">
                    <div class="panel-body">
                       <input type="button" name="previous" class=" previousStep btn btn-danger" value="Volver" />
                       <input type="button" name="next" class=" nextStep btn btn-primary" value="Siguiente" data-siguiente="datos-reserva" />
                    </div>
                </div>    
              <?php 
                $data_view = array(
                               ...
                               <div id="collapse-politicas" class="panel-collapse collapse in">
                                <div class="panel-body">
                                    <div class="form-group">
                                      ...
                                      <div class="radio">
                                        <label><input type="radio" name="tipopolitica" class="tipopolitica" id="tipopoliticaA" value="0" <?=$checkedPolitica== 0?'checked':'';?> > Política Stantard (Políticas pre-establecidas por Inca Lake)</label>
                                      </div>
                                      <div class="radio">
                                        <label><input type="radio" name="tipopolitica" class="tipopolitica" id="tipopoliticaB" value="1" <?=$checkedPolitica== 1?'checked':'';?> > Política Personalizada (Política personalizada para cada actividad)</label>
                                      </div>
                                    </div>

                                      <div class="form-group">
                                        <label class="control-label">DESCRIPCIÓN DE LAS POLÍTICAS PARA LA ACTIVIDAD: </label>
<!--
...
-->
                                        <textarea class="form-control txt_politicas" id="txt_politicas" name="politicas" rows="7"><?=@$texto_politicas;?></textarea>
                                      </div>
                                    <div style="padding-top:10px" class="col-md-12 form-horizontal">
                                     <?php
                                        ...
                                        if(!empty($tiempo_anticipacion))$tiempo_anticipacion = explode(':',$tiempo_anticipacion);
                               ...
                               <div class="row">
                                <div class="col-md-6">    
                                  <div class="panel panel-success">
                                    <div class="panel-heading"><strong><span class="badge" title="Paso 2"><big>2</big></span> INDIQUE TIEMPO DE ANTICIPACIÓN PARA LA RESERVA</strong></div>
                                    <div class="panel-body">
                                      <div class="form-group">
                                        <label class="control-label">Tiempo de anticipación: </label>
                                        <input value="<?=@$tiempo_anticipacion[0];?>" onchange="if(this.value.length)$('#tip_antc').attr('required','required');else $('#tip_antc').removeAttr('required')" type="number" name="anticipacion_reserva" class="form-control" />
                                      </div>
                                      <div class="form-group">
                                        <label class="control-label">Seleccione tiempo en:</label>
                                        <select name="anticipacion_reserva_tipo" class="form-control" <?=@$producto['anticipacion_reserva_producto']?'required':'';?> id="tip_antc">
                                          <option value="">-- Seleccione --</option>
                                          <?=$options_tiempo;?>
                                        </select>
                                      </div>
                              ...
                              ?>                                        
                                  ...
                                  ?>
                                    ...
                                    <label style="cursor:pointer;">
                                        <input type="radio" name="forms_multiple" value="0" <?=@$producto['forms_multiple']? '' : 'checked';?> >
                                         Requerir datos solo del lider
                                    </label> <br>
                                    <label style="cursor: pointer;">
                                        <input type="radio" name="forms_multiple" value="1" <?=@$producto['forms_multiple']? 'checked' : '';?> > 
                                    ...
                                    <!--    <strong>Requerir datos solo del Lider</strong></label>
                                      ...
                                      <?php endif; ?>
                                          <label style="font-weight: normal;display: block;">
                                              <input type="checkbox" name="inputs[]" value="<?= $form['input_id'] ?>"  
                                              <?php if($form['checked'] == "true"): ?> checked <?php endif; ?>/> 
                                              <?= $input->{'es'} ?>
                                          </label>             
                              <?php endforeach; ?>
                          </div></div></div>
          ...
          <div class="panel panel-primary navform">
               <div class="panel-body">
                         <input type="button" name="previous" class="previousStep btn btn-danger" value="Volver" />
                         <input type="submit" id="btnGuardar" name="formButton" style="float:right" class="btn btn-success" value="<?=$formButton;?>" />
               </div>
          </div>
           <div class="headline">
                 <div>Paso 6: Opciones para reservar</div>
              ...
              <div class="col-md-6">
                                  <div class="panel panel-danger">
                                    <div class="panel-heading"><strong><span class="badge" title="Paso 4"><big>1</big></span> INDIQUE OPCIONES DE RECOJO AL CLIENTE:</strong></div>
                                    <div class="panel-body">
                                      <div class="form-group">
                                        <label class="control-label">Indique tipo de recojo: </label>
                                        <div class="radio">
                                          <label><input type="radio" name="tiporecojo" class="tiporecojo" value="0" <?=$checkedRecojo==0?'checked':'';?> > Punto de presentación</label>
                                        </div>
                                        <div class="container" id="container-punto-de-presentacion" style="display: <?=$checkedRecojo==0?'block':'none'?>;">
                                          <div class="radio">
                                            <label><input type="radio" name="tiporecojoA" class="tiporecojoA" value="0" > Los clientes deben presentarse en Plaza de armas de la ciudad</label>
                                          </div>
                                          <div class="radio">
                                            <label><input type="radio" name="tiporecojoA" class="tiporecojoA" value="1" > Indicar donde debe presentarse el cliente</label>
                                          </div>
                                        </div>
                                        <div class="radio">
                                          <label><input type="radio" name="tiporecojo" class="tiporecojo" value="1" <?=$checkedRecojo==1?'checked':'';?> > Recojo de hotel</label>
                                        </div>
                                        <div class="container" id="container-recojo-de-hotel" style="display: <?=$checkedRecojo==1?'block':'none'?>;">
                                          <div class="radio">
                                            <label><input type="radio" name="tiporecojoB" class="tiporecojoB" value="0" > Plaza de armas</label>
                                          </div>
                                          <div class="radio">
                                            <label><input type="radio" name="tiporecojoB" class="tiporecojoB" value="1" > Otros</label>
                                          </div>
                                        </div>
                                      </div>
                                      <div class="form-group">
                                        <label class="control-label" id="lbl_lugar_encuentro">Indique hoteles aplicables para el recojo:</label>
                                        <textarea class="form-control" row="1" name="txt_lugar_encuentro" id="txt_lugar_encuentro"><?=@$producto['lugar_recojo'];?></textarea>
                                      </div>
                                      <div class="form-group">
                                        <label class="control-label">Indique donde terminará la actividad (opcional):</label>
                                        <textarea class="form-control" rows="1" name="txt_lugar_finalizacion" id="txt_lugar_finalizacion" placeholder="Describa donde se les dejará a los clientes después de finalizar la excursión"><?=@$producto['lugar_finalizacion_actividad'];?></textarea>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              ...
                              <div class="col-md-6">
                                <div class="panel panel-warning">
                                    <div class="panel-heading"><strong><span class="badge" title="Paso 3"><big>2</big></span> AFORO y REQUERIMIENTO DE DATOS PARA LA RESERVA</strong></div>
                                    <div class="panel-body">
                                       <div class="form-group">
                                        <label class="control-label">Aforo (cantidad máxima para reservar): </label>
                                        <input value="<?=@$producto['capacidad'] ? @$producto['capacidad']: '30' ;?>" min="1" type="number" name="aforo" class="form-control" />
                                      </div>
                                      <div class="form-group">
                                        <label class="control-label">Para reservar requerir datos: </label>
                                        <select name="requerimiento_datos" class="form-control" id="slct_requerimiento_datos">
                                             <?=$options_requerir;?>
                                        </select>
                                      </div>
                                ...
                                ?>
                                <div class="panel panel-success">
                                    <div class="panel-heading"><strong><span class="badge" title="Paso 3"><big>3</big></span> ASOCIAR GUIAS A LA ACTIVIDAD/SERVICIO (OPCIONAL)</strong></div>
                                    <div class="panel-body">
                                      <?php 
                                      ...
                                      <div id="selected-tipoguias" style="display:<?=$checkboxSelected == 3 || $checkboxSelected == 4?'none':'block';?>" >
                                        ...
                                        <select multiple name="slct_guias_seleccionados[]" id="slct_guias_seleccionados">
                                          <?php foreach ($arrayGuiasList as $key => $value): ?>
                                            ...
                                            ?>
                                            <option value="<?=$text;?>" <?=$value['selected']?'selected':'';?> ><?=$value['descripcion'];?></option>      
                                          <?php endforeach ?>
                                        </select>
                                      </div>
                                      <br/><br/>
...
$('.addGaleriaSlider').click(function(event){
  ...
  if(contenedorGaleria.eq(0).find('.galeriaDivs').length)contenedorGaleria.eq(0).append(html);else contenedorGaleria.eq(0).html(html);
  return false;
});
$('.addGaleriaAdjunto').click(function(event){
  var html = '<?=addFormGaleria(null,null,0);?>';
  event.stopPropagation();
  if(contenedorGaleria.eq(1).find('.galeriaDivs').length)contenedorGaleria.eq(1).append(html);else contenedorGaleria.eq(1).html(html);
  return false;
});
...
jQuery(document).ready(function($) {
  ...
  var rdbtnPoliticaSelected = null,rdbtnRecojoSelected = null;
  ...
  */
  ...
  if ( parseInt(data_editar) === 0 ) {
    $.ajax({
      ...
      type: 'POST',
      dataType: 'JSON',
      data: {lang: idioma_actividad,idservicio: id_servicio},
    }).done(function(data) {
      //console.log(data);
      if (data.length != 0 ) {
