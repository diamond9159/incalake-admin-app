  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        ...
        <?php 
          //echo json_encode($id_servicio_referencia);
                               ...
                               <!--  <div class="panel panel-info">
                                 ...
                                 -->
                                    ...
                                    <div class="form-group col-md-12">
                                      ...
                                      <small>
                                        <label class="checkbox-inline"><input type="checkbox" name="chckbx_dia" checked value="0" id="chckbx_dia_0" class="chckbx_dia">Lunes</label>
                                        <label class="checkbox-inline"><input type="checkbox" name="chckbx_dia" checked value="1" id="chckbx_dia_1" class="chckbx_dia">Mártes</label>
                                        <label class="checkbox-inline"><input type="checkbox" name="chckbx_dia" checked value="2" id="chckbx_dia_2" class="chckbx_dia">Miércoles</label>
                                        <label class="checkbox-inline"><input type="checkbox" name="chckbx_dia" checked value="3" id="chckbx_dia_3" class="chckbx_dia">Jueves</label>
                                        <label class="checkbox-inline"><input type="checkbox" name="chckbx_dia" checked value="4" id="chckbx_dia_4" class="chckbx_dia">Viérnes</label>
                                        <label class="checkbox-inline"><input type="checkbox" name="chckbx_dia" checked value="5" id="chckbx_dia_5" class="chckbx_dia">Sábado</label>
                                        <label class="checkbox-inline"><input type="checkbox" name="chckbx_dia" checked value="6" id="chckbx_dia_6" class="chckbx_dia">Domingo</label>
                                      </small>
                                    </div>
    ...
    <a href="<?=base_url().'admin/servicio';?>" class="btn btn-danger"><b><span class="fa fa-chevron-left"></span> VOLVER</b></a><br/><br/><br/>
    </div>
  </div>
</form>
...
<style type="text/css">.fc-nonbusiness { background:#e74c3c;}</style>
...
jQuery(document).ready(function($) {
  array_dias_no_activos = new Array();
  var eventos = { items: [] };
  $('#calendario_incalake').fullCalendar({
    ...
    select: function(fstart, fend) {
      ...
      //var end          = moment( array_end[0]+"-"+array_end[1]+"-"+( parseInt(array_end[2]) -1 ) ,"YYYY-MM-DD").format("YYYY-MM-DD")+ " 23:59:59";
...
function actualizar_eventos(){
  ...
  $.each(js_bloqueo.get_datos_bloqueo(), function(index, val) {
    html_bloqueo += '<li class="list-group-item">'+val.title+'<span class="pull-right fa fa-close text-danger btn btn-delete-objeto" data-id="'+val.start+';'+val.end+'" data-tipo="0" data-idobjeto="'+val.id+'" title="Eliminar Fecha Bloqueada"></span></li>';
...
$(document).on('change', '#slct_disponibilidad', function(event) {
  ...
  js_disponibilidad.get_eliminar_disponibilidad();
  js_bloqueo.get_vaciar_bloqueo();
  js_oferta.get_vaciar_oferta();
  cantidad_dias_inicio_disponibilidad = '0';
  cantidad_dias_fin_disponibilidad = '0';
  $('#calendario_incalake').fullCalendar( 'next' );
  $('#calendario_incalake').fullCalendar( 'prev' ); 
...
function actualizar_calendario(id_producto) {
    ...
    $.ajax({
      ...
      success: function(data) {
        //console.log(data);
        var response_data_disponibilidad = data.disponibilidad;
        var response_data_bloqueo        = data.bloqueo;
        var response_data_oferta         = data.oferta;
        
        var temp_response_data_disponibilidad = JSON.parse(response_data_disponibilidad);
        
        if (temp_response_data_disponibilidad.lenght) {
          array_dias_no_activos.length = 0;
          array_dias_no_activos = temp_response_data_disponibilidad[0]['dias_no_activos'];
        ...
        $.each(JSON.parse(response_data_disponibilidad), function(index, val) {
        ...
        $.each(JSON.parse(response_data_bloqueo), function(index, val) {
          js_bloqueo.set_datos_bloqueo( val );
        });

        $.each(JSON.parse(response_data_oferta), function(index, val) {
