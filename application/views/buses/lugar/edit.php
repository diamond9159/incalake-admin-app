...
<div class="container-fluid">
	<div class="row">
		<div class="col-md-4 col-md-offset-2 col-sm-4 col-md-offset-2">
			<h3 class="text-primary"><span class="fa fa-edit"></span> EDITAR LUGAR</h3>
			<?php echo form_open('buses/lugar/edit/'.$lugar['id_lugar'],array("class"=>"form-horizontal","id"=>"form_edit_lugar")); ?>
				<div class="form-group">
					<label for="nombre_lugar" class="col-md-12">Nombre Lugar <span class="text-danger fa fa-asterisk"></span></label>
					<div class="col-md-12">
						<input type="text" name="nombre_lugar" value="<?php echo ($this->input->post('nombre_lugar') ? $this->input->post('nombre_lugar') : $lugar['nombre_lugar']); ?>" class="form-control" id="nombre_lugar" />
				...
				<div class="form-group">
					...
					<div class="col-md-12">
						<input type="text" name="coordenadas" value="<?php echo ($this->input->post('coordenadas') ? $this->input->post('coordenadas') : $lugar['coordenadas']); ?>" class="form-control" id="coordenadas"  readonly/>
					...
					<div class="col-md-12">
						<select name="tipo_lugar" class="form-control validate[required]" id="tipo_lugar">
							...
							foreach($all_types as $key => $at){
								$selected = ($key == (Integer)$lugar['tipo_lugar']) ? ' selected="selected"' : "";
								echo '<option value="'.$key.'" '.$selected.' >'.$at.'</option>';
							} 
							?>
						</select>
		...
		<div class="form-group">
			<div class="col-md-12 text-center">
	        	...
	        	<a href="<?=base_url()?>buses/lugar/" class="btn btn-danger">VOLVER</a>
	        </div>
		</div>
			<?php echo form_close(); ?>
	</div>
...
<script type="text/javascript">
  var coordenadasLugar = '<?=@$lugar["coordenadas"]?>';
  var arrayCoordenadasLugar = [];
  if (coordenadasLugar.length > 0 ) {
  	arrayCoordenadasLugar = coordenadasLugar.split(',');
  }
  var map = null;
  function initMap() {
  	var puno = {lat: parseFloat(arrayCoordenadasLugar[0]),lng:parseFloat(arrayCoordenadasLugar[1])};
    ...
    if ( arrayCoordenadasLugar.length > 0 ) {
	    marker.setPosition( {lat: parseFloat(arrayCoordenadasLugar[0]),lng:parseFloat(arrayCoordenadasLugar[1])} );    	
    }
  ...
  jQuery(document).ready(function($) {
    $("#form_edit_lugar").validationEngine('attach', {
		relative: true,
